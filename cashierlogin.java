...
Connection cn=connection.mycon();
Statement st=cn.createStatement(ResultSet.TYPE_SCROLL_INSENSITIVE,ResultSet.CONCUR_UPDATABLE);
ResultSet rs=st.executeQuery("select * from cashierdata where name='"+a+"'  and password='"+b+"'");
boolean ss=rs.next();
if(ss)
{
clock();
s_no();
ResultSet rs1=st.executeQuery("select cashier_id from cashierdata where name='"+a+"'  and password='"+b+"'");

if(rs1.next())
{
int c=rs1.getInt(1);
st.executeUpdate("insert into cashierlogindata values('"+cec1+"','"+c+"','"+a+"','"+time+"','"+time+"','"+date+"')");
}
new cashier();
dispose();
}
